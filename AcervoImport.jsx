...
import { executeAcervoImport, inspectAcervoImport } from './services/acervo/acervoService.js';
...
export default function AcervoImport() {
  ...
  const runImport = async () => {
    ...
    try {
      const done = await executeAcervoImport({ plan, onProgress: setProgress });
      setResult(done);
    } catch (e) {
  ...
  return <><Navbar/><main className="min-h-screen bg-background px-4 pb-24 pt-6 text-foreground sm:px-6 lg:px-8">
    <div className="mx-auto max-w-6xl">
      ...
      {plan && <section className="mt-5 rounded-3xl border border-border bg-card p-5">
        <div className="flex flex-wrap items-center gap-3"><div className="rounded-xl bg-primary/10 p-2 text-primary"><CheckCircle2 className="h-5 w-5"/></div><div><h2 className="text-sm font-black">Prévia da importação</h2><p className="text-xs text-muted-foreground">{plan.entries.length} entidades · {plan.images.length} imagens · {issueCount} problemas</p></div><span className="ml-auto rounded-full border border-border px-3 py-1 text-[10px] font-black">{issueCount ? 'REVISAR' : 'PRONTO'}</span></div>
      ...
      {result && <section className="mt-5 rounded-3xl border border-primary/30 bg-primary/5 p-6"><h2 className="text-lg font-black">Importação concluída</h2><p className="mt-2 text-sm text-muted-foreground">Coleção <b>{result.collectionId}</b> processada com {result.created} novas cartas, {result.updated} atualizadas e {result.uploaded} imagens vinculadas.</p><Link to={`/acervo/${encodeURIComponent(result.collectionId)}`} className="mt-4 inline-flex rounded-xl bg-primary px-4 py-2 text-xs font-black text-primary-foreground">Abrir coleção</Link></section>}
    </div>
  </main></>;
}
