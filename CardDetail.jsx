...
import { ArrowLeft, ImageOff, Lock, Package, Shield, Sparkles, Swords, UserRound } from 'lucide-react';
...
export default function CardDetail() {
  ...
  const description = selectedForm?.description || entity.description || entity.lore || '';
  const statRows = [
    ['ATK', entity.atk ?? entity.attack],
    ['DEF', entity.def ?? entity.defense],
    ['MAG', entity.mag],
    ['SPD', entity.speed],
    ['HP', entity.hp],
      ...
      <main className="mx-auto w-full max-w-6xl px-4 pb-28 pt-6 sm:px-6 sm:pt-10 lg:px-8">
        ...
        <div className="grid gap-7 lg:grid-cols-[minmax(0,420px)_1fr] lg:gap-10">
          <section>
            ...
            {forms.length > 0 && <div className="mt-4 rounded-2xl border border-border bg-card p-3"><div className="mb-3 flex items-center gap-2 px-1 text-xs font-extrabold uppercase tracking-[.14em] text-muted-foreground"><Sparkles className="h-4 w-4 text-primary" /> Formas disponíveis</div><div className="flex gap-2 overflow-x-auto pb-1"><FormButton label="Base" active={selectedFormId === 'base'} onClick={() => setSelectedFormId('base')} />{forms.map((form) => <FormButton key={form.id} label={form.name} locked={form.locked} active={selectedFormId === form.id} onClick={() => !form.locked && setSelectedFormId(form.id)} />)}</div></div>}
          </section>
          ...
          <section className="min-w-0 lg:pt-3">
            ...
            {statRows.length > 0 && <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5">{statRows.map(([label, value]) => <Stat key={label} label={label} value={value} />)}</div>}

            {description && <div className="mt-7 rounded-2xl border border-border bg-card p-5 sm:p-6"><h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-[.14em]"><Shield className="h-4 w-4 text-primary" /> Sobre</h2><p className="mt-3 whitespace-pre-line text-sm leading-7 text-muted-foreground">{description}</p></div>}

            {forms.length > 0 && <div className="mt-5 rounded-2xl border border-border bg-card p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div><h2 className="text-sm font-black uppercase tracking-[.14em]">Transformações</h2><p className="mt-1 text-xs text-muted-foreground">A entidade-base permanece única; as formas são estados vinculados a ela.</p></div><Sparkles className="h-5 w-5 shrink-0 text-primary" /></div><div className="mt-4 space-y-2"><ProgressRow name="Base" unlocked />{forms.map((form) => <ProgressRow key={form.id} name={form.name} unlocked={!form.locked} />)}</div></div>}
          </section>
        </div>
      </main>
    </div>
...
function Stat({ label, value }) { return <div className="rounded-xl border border-border bg-card p-3 text-center"><div className="text-lg font-black">{value}</div><div className="text-[9px] font-black uppercase tracking-[.12em] text-muted-foreground">{label}</div></div>; }
function ProgressRow({ name, unlocked }) { return <div className="flex min-h-11 items-center justify-between gap-3 rounded-xl border border-border bg-background/60 px-3"><span className="text-sm font-bold">{name}</span>{unlocked ? <span className="text-[11px] font-extrabold text-emerald-300">Disponível</span> : <span className="flex items-center gap-1.5 text-[11px] font-extrabold text-muted-foreground"><Lock className="h-3.5 w-3.5" /> Bloqueada</span>}</div>; }
