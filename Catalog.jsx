      ...
      <main className="mx-auto w-full max-w-[1480px] px-4 pb-28 pt-4 sm:px-6 sm:pt-7 lg:px-8">
        ...
        <section className="mt-5 rounded-2xl border border-border bg-card/75 p-3 sm:p-4">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-[minmax(240px,1fr)_minmax(180px,260px)_170px_160px]">
            <label className="relative min-w-0"><Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input aria-label={`Pesquisar ${TYPE_CONFIG[type].label.toLowerCase()}`} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Pesquisar ${TYPE_CONFIG[type].label.toLowerCase()}...`} className="h-12 w-full rounded-xl border border-border bg-background pl-10 pr-4 text-sm outline-none focus:border-primary/60" /></label>
            <select aria-label="Filtrar por coleção" value={collectionFilter} onChange={(event) => setCollectionFilter(event.target.value)} className="h-12 rounded-xl border border-border bg-background px-3 text-sm font-semibold"><option value="all">Todas as coleções</option>{uniqueCollections.map((collection) => <option key={collection.id} value={String(collection.id)}>{getName(collection)}</option>)}</select>
            <select aria-label="Filtrar por raridade" value={rarity} onChange={(event) => setRarity(event.target.value)} className="h-12 rounded-xl border border-border bg-background px-3 text-sm font-semibold"><option value="all">Todas as raridades</option>{CATALOG_RARITIES.map((value) => <option key={value} value={value}>{value}</option>)}<option value="unreviewed">Em revisão</option></select>
            <select aria-label="Ordenar catálogo" value={sort} onChange={(event) => setSort(event.target.value)} className="h-12 rounded-xl border border-border bg-background px-3 text-sm font-semibold"><option value="name-asc">Nome A–Z</option><option value="name-desc">Nome Z–A</option><option value="rarity-desc">Maior raridade</option></select>
          </div>
        </section>
