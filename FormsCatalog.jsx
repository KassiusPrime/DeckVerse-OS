...
export default function FormsCatalog() {
  ...
  const snapshot = snapshotQuery.data || { collections: [], characters: [], bosses: [] };
  ...
  const filtered = useMemo(() => {
    ...
    return forms.filter((form) => {
      ...
      return [form.name, form.baseName, form.entityType, collectionNames.get(String(form.collectionCode)), form.description].some((value) => normalize(value).includes(needle));
    });
      ...
      <main className="mx-auto w-full max-w-[1480px] px-4 pb-28 pt-8 sm:px-6 lg:px-8">
        <section className="overflow-hidden rounded-3xl border border-border bg-card/75 p-6 sm:p-8 lg:p-10">
          <div className="grid gap-6 lg:grid-cols-[1fr_minmax(320px,520px)] lg:items-end">
            <div><div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[.17em] text-primary"><Sparkles className="h-4 w-4" /> Estados colecionáveis</div><h1 className="mt-2 text-3xl font-black tracking-[-.04em] sm:text-5xl">Formas & Transformações</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-muted-foreground sm:text-base">Transformações, evoluções e estados permanecem ligados à entidade-base. Personagens continuam Personagens e Bosses continuam Bosses; uma forma não cria uma segunda entidade-base.</p></div>
            <div className="grid gap-2 sm:grid-cols-[1fr_190px]">
              <label className="relative"><Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input aria-label="Buscar forma ou entidade" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar forma ou entidade..." className="h-12 w-full rounded-2xl border border-border bg-background/80 pl-11 pr-4 text-sm outline-none focus:border-primary/60" /></label>
        ...
        {snapshotQuery.isLoading ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">{Array.from({ length: 12 }).map((_, index) => <div key={index} className="aspect-[4/5] animate-pulse rounded-2xl border border-border bg-card" />)}</div> : filtered.length > 0 ? (
          <section className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
            {filtered.map((form, index) => {
              ...
              const baseTypeLabel = form.entityType === 'boss' ? 'Boss' : 'Personagem';
              const targetId = form.baseEntityId || form.baseCharacterId;
              const card = <div className="group overflow-hidden rounded-2xl border border-border/80 bg-card transition duration-200 hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-[0_18px_48px_rgba(0,0,0,.25)]"><div className="relative aspect-[4/5] overflow-hidden bg-muted"><Media src={form.image_url || form.imageUrl || ''} alt={form.name} /><div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-black/95 via-black/38 to-transparent" /><div className="absolute inset-x-3 bottom-3"><div className="text-[9px] font-black uppercase tracking-[.14em] text-primary">{collectionName} · {baseTypeLabel}</div><h2 className="mt-1 line-clamp-2 text-sm font-black leading-tight text-white">{form.name}</h2><p className="mt-1 truncate text-[10px] text-white/55">{form.baseName || 'Entidade-base'}</p></div></div></div>;
              return targetId ? <Link key={form.id || index} to={`/card/${encodeURIComponent(targetId)}`}>{card}</Link> : <div key={form.id || index}>{card}</div>;
            })}
          </section>
