...
export default function Navbar({ onSearch }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/80 bg-background/90 backdrop-blur-xl">
      <nav className="mx-auto flex min-h-16 w-full max-w-[1480px] items-center gap-2 px-3 sm:gap-3 sm:px-6 lg:px-8" aria-label="Navegação principal">
        ...
        <form onSubmit={submitSearch} className="ml-1 hidden min-w-0 flex-1 md:block xl:max-w-sm"><label className="relative block"><Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input aria-label="Buscar em todo o DeckVerse" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar no DeckVerse" className="h-10 w-full rounded-xl border border-border bg-card pl-10 pr-3 text-sm text-foreground outline-none transition placeholder:text-muted-foreground/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/15" /></label></form>
        <div className="ml-auto hidden items-center gap-1 xl:flex">
          ...
          {isAdmin && <NavLink to="/acervo" className={linkClass}><BookOpen className="h-4 w-4" />Acervo</NavLink>}
          {isAdmin && <NavLink to="/admin/content" className={linkClass}><Sparkles className="h-4 w-4" />Conteúdo</NavLink>}
      ...
      {mobileOpen && <div ref={menuRef} className="border-t border-border bg-background px-3 py-4 shadow-2xl sm:px-6 xl:hidden"><form onSubmit={submitSearch} className="mb-3 md:hidden"><label className="relative block"><Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input aria-label="Buscar em todo o DeckVerse" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar no DeckVerse" className="h-11 w-full rounded-xl border border-border bg-card pl-10 pr-3 text-sm text-foreground outline-none focus:border-primary/60" /></label></form><div className="grid gap-1 sm:grid-cols-2">{NAV_LINKS.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} onClick={() => setMobileOpen(false)} className={linkClass}><Icon className="h-4 w-4" />{label}</NavLink>)}<NavLink to={isAuthenticated ? '/profile' : '/login'} onClick={() => setMobileOpen(false)} className={linkClass}>{isAuthenticated ? <UserRound className="h-4 w-4" /> : <LogIn className="h-4 w-4" />}{isAuthenticated ? 'Perfil' : 'Entrar com Discord'}</NavLink>{isAdmin && <NavLink to="/acervo" onClick={() => setMobileOpen(false)} className={linkClass}><BookOpen className="h-4 w-4" />Acervo</NavLink>}{isAdmin && <NavLink to="/admin/content" onClick={() => setMobileOpen(false)} className={linkClass}><Sparkles className="h-4 w-4" />Central de Conteúdo</NavLink>}{isAdmin && <NavLink to="/admin/card-values" onClick={() => setMobileOpen(false)} className={linkClass}><Coins className="h-4 w-4" />Valores das cartas</NavLink>}{isAdmin && <NavLink to="/admin/synopses" onClick={() => setMobileOpen(false)} className={linkClass}><BookOpen className="h-4 w-4" />Sinopses</NavLink>}{isAdmin && <NavLink to="/admin" onClick={() => setMobileOpen(false)} className={linkClass}><ShieldCheck className="h-4 w-4" />Admin</NavLink>}</div></div>}
    </header>
  );
}
