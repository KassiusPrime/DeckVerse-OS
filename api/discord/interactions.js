...
function message(content, embeds = [], components = [], ephemeral = false) {
  return { type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE, data: { content, embeds, components, ...(ephemeral ? { flags: 64 } : {}) } };
}

function option(options, name) { return (options || []).find((entry) => entry.name === name); }
...
async function inventoryPage(supabase, discordId, page = 0) {
  ...
  const { data, error, count } = await supabase.from('rosters')
    .select('copies, cards(name, rarity, entity_type, image_url, collections(name))', { count: 'exact' })
    .eq('profile_id', profile.id).order('acquired_at', { ascending: false }).range(from, to);
  if (error) throw error;
...
function inventoryEmbed(result) {
  return {
    ...
    fields: result.rows.map((row) => ({ name: `${row.cards?.rarity || '—'} · ${row.cards?.name || 'Entidade'}`, value: `${row.cards?.collections?.name || 'DeckVerse'} · ${row.copies}x`, inline: false })),
...
async function runRoll(interaction, supabase, discordId) {
  ...
  return message('', [{ title: count === 1 ? 'Roll concluído' : `${count} rolls concluídos`, description: `${preview}${pulls.length > 10 ? `\n…e mais ${pulls.length - 10}` : ''}`, image: top?.image_url ? { url: top.image_url } : undefined, color: 0x7c5cff, footer: { text: `Pity ${data.pity_before} → ${data.pity_after} · custo ${data.cost}` } }]);
}
...
async function claimBySlot(interaction, supabase, discordId) {
  ...
  const { data: wave, error: waveError } = await supabase.from('discord_spawn_waves').select('id').eq('guild_id', guildId).eq('status', 'open').gt('expires_at', new Date().toISOString()).order('created_at', { ascending: false }).limit(1).maybeSingle();
  if (waveError) throw waveError;
  if (!wave) return message('Não há spawn aberto neste servidor.', [], [], true);
  let query = supabase.from('discord_spawn_cards').select('id, slot_index').eq('wave_id', wave.id).is('claimed_by_profile_id', null).order('slot_index').limit(1);
  ...
  } catch (claimError) { return message(errorText(claimError), [], [], true); }
}
...
async function handleSpawnSettings(interaction, supabase) {
  ...
  if (sub.name === 'ch') { const channelId = option(sub.options, 'c')?.value; await updateGuildSpawnSettings(supabase, guildId, { spawn_channel_id: channelId }); return message(`✅ Canal de spawn definido para <#${channelId}>.`, [], [], true); }
  if (sub.name === 'on') { const settings = await ensureGuildSettings(supabase, guildId); if (!settings.spawn_channel_id) return message('Defina o canal primeiro com `/s ch`.', [], [], true); await updateGuildSpawnSettings(supabase, guildId, { auto_spawn_enabled: true, next_spawn_at: new Date().toISOString() }); return message('✅ Spawn automático ligado.', [], [], true); }
  if (sub.name === 'off') { await updateGuildSpawnSettings(supabase, guildId, { auto_spawn_enabled: false }); return message('⏸️ Spawn automático desligado.', [], [], true); }
  if (sub.name === 't') { const minutes = Math.max(5, Math.min(1440, Number(option(sub.options, 'm')?.value || 30))); await updateGuildSpawnSettings(supabase, guildId, { spawn_interval_minutes: minutes }); return message(`✅ Intervalo: **${minutes} min**.`, [], [], true); }
  if (sub.name === 'max') { const max = Math.max(1, Math.min(100, Number(option(sub.options, 'n')?.value || 100))); await updateGuildSpawnSettings(supabase, guildId, { max_cards_per_wave: max }); return message(`✅ Máximo por rodada: **${max} cartas**.`, [], [], true); }
  if (sub.name === 'now') { await scheduleSpawnNow(supabase, guildId); return message('✅ Spawn agendado para o próximo ciclo automático.', [], [], true); }
  return message('Configuração não reconhecida.', [], [], true);
}

async function handleDeckCommand(interaction, supabase, discordId) {
  const sub = interaction.data?.options?.[0]?.name || 'ajuda';
  try {
    if (sub === 'listar') {
      const decks = await listDecks(supabase, discordId);
      if (!decks.length) return message('Você ainda não possui decks invocáveis. Crie um deck no DeckVerse e habilite a invocação pelo Discord.', [], [], true);
      const lines = decks.slice(0, 20).map((deck) => `${deck.is_primary ? '⭐ ' : ''}**${deck.name}** · \`${deck.invocation_code}\` · ${deck.card_count} cartas`);
      return message('', [{ title: '🃏 Seus decks invocáveis', description: lines.join('\n'), color: 0x5865f2, footer: { text: 'Use /deck invocar codigo:DV-XXXXXXXX' } }], [], true);
    }
    if (sub === 'ver') {
      const code = option(interaction.data?.options?.[0]?.options, 'codigo')?.value;
      const deck = await getDeck(supabase, discordId, code);
      return message('', [deckEmbed(deck)], [], true);
    }
    if (sub === 'invocar') {
      const code = option(interaction.data?.options?.[0]?.options, 'codigo')?.value;
      const deck = code ? await getDeck(supabase, discordId, code) : await getPrimaryDeck(supabase, discordId);
      return message(`🌌 **${deck.name}** foi invocado.`, [deckEmbed(deck, 'invoke')]);
    }
    if (sub === 'principal') {
      const deck = await getPrimaryDeck(supabase, discordId);
      return message(`⭐ Seu deck principal é **${deck.name}**.`, [deckEmbed(deck, 'invoke')]);
    }
    return message('**DeckVerse · Invocação Discord**\n`/deck listar` — seus decks\n`/deck ver codigo:DV-XXXXXXXX` — visualizar\n`/deck invocar` — invocar o principal\n`/deck invocar codigo:DV-XXXXXXXX` — invocar um deck específico\n`/deck principal` — visualizar/invocar o principal.', [], [], true);
  } catch (error) {
    return message(deckErrorText(error), [], [], true);
  }
}
...
async function handleCommand(interaction, supabase) {
  ...
  if (interaction.guild_id) { try { await touchGuildPlayer(supabase, interaction, discordId); } catch (error) { console.warn('[DeckVerse Discord] guild touch failed', error?.message || error); } }
  ...
  if (name === 'deck') return handleDeckCommand(interaction, supabase, discordId);
  ...
  if (name === 'h' || name === 'support') return message('`/r` roll · `/c` pegar · `/i` inventário · `/p` perfil · `/deck` decks · `/s` spawn (admin).\nEx.: `/r q:10`, `/deck listar`, `/deck invocar`, `/s ch`, `/s on`.', [], [], true);
  return message('Comando não reconhecido.', [], [], true);
}

function disableSpawnButton(components, customId, username) {
  return (components || []).map((row) => ({ ...row, components: (row.components || []).map((button) => button.custom_id === customId ? { ...button, disabled: true, style: 2, label: `✅ ${username || 'Pegou'}`.slice(0, 80) } : button) }));
}
...
async function handleComponent(interaction, supabase) {
  ...
  if (interaction.guild_id) { try { await touchGuildPlayer(supabase, interaction, clicker); } catch (error) { console.warn('[DeckVerse Discord] guild touch failed', error?.message || error); } }
  if (customId.startsWith('spawnclaim:')) {
    ...
    try {
      ...
      return { type: InteractionResponseType.UPDATE_MESSAGE, data: { components: disableSpawnButton(interaction.message?.components, customId, username) } };
    } catch (error) { return message(errorText(error), [], [], true); }
  }
  if (customId.startsWith('inventory:')) {
    ...
    return { type: InteractionResponseType.UPDATE_MESSAGE, data: { embeds: [inventoryEmbed(result)], components: paginator(ownerDiscordId, result.page, result.totalPages) } };
  }
