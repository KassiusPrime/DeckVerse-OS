...
const DEFAULT_GUILD_ID = '1487497709148569610';
...
export default async function handler(req, res) {
  ...
  const guildId = process.env.DISCORD_GUILD_ID || process.env.GUILD_ID || DEFAULT_GUILD_ID;
  const endpoint = `https://discord.com/api/v10/applications/${APP_ID}/guilds/${guildId}/commands`;
  ...
  return res.status(200).json({ ok: true, scope: 'guild', guild_id: guildId, count: Array.isArray(body) ? body.length : commands.length, commands: commands.map((c) => c.name) });
}
