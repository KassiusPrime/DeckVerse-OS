...
export function validateCollection(input = {}) {
  ...
  const aliases = Array.isArray(input.aliases)
    ? input.aliases.map(a => String(a).trim().toUpperCase()).filter(Boolean)
    : typeof input.aliases === "string"
    ? input.aliases.split(",").map(a => a.trim().toUpperCase()).filter(Boolean)
    : [];

  const category = input.category || input.bank || "";
  const registrySource = input.registrySource || undefined;
...
export function validateCard(input = {}) {
  ...
  const canonical = {
    name,
    species: input.species || input.race || "",
  ...
  const gameplay = {
    rarity,
    role,
    stats: { attack, defense, speed, hp, mag },
    cost: input.cost || 1,
    affinity: natures[0] || (typeof input.element === "string" ? input.element : "Void"),
    skills,
    is_boss
  };
  ...
  return {
    ...
    data: {
      ...
      element: natures[0] || (typeof input.element === "string" ? input.element : undefined),
      personalities,
      personality,
      power_origins,
