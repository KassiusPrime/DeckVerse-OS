...
function synopsisOrError(input, kind, errors, { required = false } = {}) {
  const value = text(input?.synopsis);
  if (!value) {
    if (required) errors.push(`Sinopse ${kind} é obrigatória.`);
    return '';
  }
  try { return assertSynopsis(value, kind); }
  catch (error) { errors.push(`Sinopse ${kind} deve respeitar o limite editorial definido.`); return value; }
}
