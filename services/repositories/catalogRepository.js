...
export async function searchCatalog(filters = {}) {
  const { data, error } = await client().rpc('admin_search_catalog', {
    p_query: clean(filters.query),
    p_kind: clean(filters.kind) || 'all',
...
export async function updateCollection(id, payload = {}) {
  const { data, error } = await client().rpc('admin_update_collection_content', {
    p_id: id,
    p_synopsis: payload.synopsis ?? null,
    p_is_active: payload.is_active ?? null,
    p_cover_url: payload.cover_url ?? null,
    p_clear_image: payload.clear_image === true,
  });
  if (error) throw error;
  return data;
}
...
export async function updateCard(id, payload = {}) {
  const { data, error } = await client().rpc('admin_update_card_content', {
    p_id: id,
    p_synopsis: payload.synopsis ?? null,
    p_is_active: payload.is_active ?? null,
...
export async function updateForm(id, payload = {}) {
  const { data, error } = await client().rpc('admin_update_form_content', {
    p_id: id,
    p_synopsis: payload.synopsis ?? null,
    p_is_active: payload.is_active ?? null,
    p_image_url: payload.image_url ?? null,
...
export const catalogRepository = Object.freeze({ searchCatalog, updateCollection, updateCard, updateForm });
