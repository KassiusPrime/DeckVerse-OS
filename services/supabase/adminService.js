...
export async function searchSynopsisTargets(query = '', kind = 'all', limit = 80) {
  ...
  if (wantsCollections) {
    ...
    jobs.push(q.then(({ data, error }) => { if (error) throw error; return (data || []).map((row) => ({ scope: 'collection', entityType: 'collection', id: row.id, name: row.name, synopsis: row.synopsis || '', description: row.description || '', imageUrl: row.cover_url || '', collectionName: row.name, category: row.category || '', isActive: row.is_active })); }));
  }
  if (wantsCards) {
    ...
    if (needle) q = q.ilike('name', `%${needle}%`);
    jobs.push(q.then(({ data, error }) => { if (error) throw error; return (data || []).map((row) => ({ scope: 'card', entityType: row.entity_type, id: row.id, name: row.name, synopsis: row.synopsis || '', description: row.description || '', imageUrl: row.image_url || '', collectionId: row.collection_id, collectionName: row.collections?.name || '', rarity: row.rarity || '', isActive: row.is_active })); }));
  }
  if (wantsForms) {
    ...
    if (needle) q = q.ilike('name', `%${needle}%`);
    jobs.push(q.then(({ data, error }) => { if (error) throw error; return (data || []).map((row) => ({ scope: 'form', entityType: 'form', id: row.id, cardId: row.card_id, name: row.name, synopsis: row.synopsis || '', description: row.description || '', imageUrl: row.image_url || '', baseName: row.cards?.name || '', collectionId: row.cards?.collection_id || '', collectionName: row.cards?.collections?.name || '', rarity: row.rarity || '', isActive: row.is_active })); }));
  }
