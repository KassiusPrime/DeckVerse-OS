...
export async function loadPublicCatalog() {
  if (!isSupabaseConfigured()) {
    ...
    const [collections, cards, items, bosses] = await Promise.all([
      db.entities.Collection.list(''), db.entities.Card.list(''), db.entities.Item.list(''), db.entities.Boss.list(''),
    ]);
    ...
    for (const card of [...(cards || []), ...(bosses || [])]) {
      for (const form of Array.isArray(card.forms) ? card.forms : []) {
        ...
        legacyForms.push({
          ...
          rarity: normalized.rarity || '', imageUrl: normalized.image_url || normalized.imageUrl || '',
          synopsis: normalized.synopsis || '', description: normalized.description || normalized.lore || '',
          order: Number(normalized.order || normalized.order_index || 1),
          collectionId: card.collection_id || card.collectionCode || '', baseName: card.name || '',
  ...
  const [collections, cards, forms] = await Promise.all([
    fetchAllPages(() => supabase.from('collections').select('id, name, synopsis, description, category, cover_url').eq('is_active', true).order('name').order('id'), 'coleções'),
    fetchAllPages(() => supabase.from('cards').select('id, collection_id, name, entity_type, rarity, role, image_url, synopsis, description, collections!inner(name, is_active)').eq('is_active', true).eq('collections.is_active', true).order('name').order('id'), 'cards'),
    fetchAllPages(() => supabase.from('card_forms').select('id, card_id, name, rarity, image_url, synopsis, description, order_index, cards!inner(name, entity_type, collection_id, is_active, collections!inner(is_active))').eq('is_active', true).eq('cards.is_active', true).eq('cards.collections.is_active', true).order('order_index').order('id'), 'formas'),
  ]);
