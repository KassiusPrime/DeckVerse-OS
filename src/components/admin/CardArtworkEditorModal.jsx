...
export default function CardArtworkEditorModal({ card, onClose, onApply }) {
  ...
  const validateFile = (candidate) => {
    ...
    if (candidate.size > MAX_FILE_BYTES) return "A imagem excede o limite de 10 MB.";
    return null;
  };
  ...
  const validateUrl = async () => {
    ...
    if (!result.ok || result.width < 1 || result.height < 1) {
      throw new Error(`Não foi possível carregar a imagem de ${provider}. O servidor pode bloquear hotlink/CORS.`);
    }
    ...
    <div className="fixed inset-0 z-[70] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="w-full max-w-2xl max-h-[92vh] overflow-y-auto rounded-xl border border-primary/40 bg-card shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-border/40">
          <div>
            <h2 className="font-heading text-sm font-bold tracking-widest">ALTERAR ARTE DA CARTA</h2>
            <p className="text-xs text-muted-foreground mt-1">{card?.name || "Carta"}</p>
          </div>
        ...
        <div className="p-5 space-y-4">
          {tab === "upload" ? (
            <label className="block cursor-pointer rounded-xl border border-dashed border-primary/40 bg-muted/10 p-8 text-center hover:bg-primary/5">
              ...
              <span className="block text-xs text-muted-foreground mt-1">JPG, PNG, WEBP ou GIF · máximo 10 MB</span>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-border/40">
            <button onClick={onClose} disabled={busy} className="px-4 py-2 text-xs font-heading border border-border/50 rounded">CANCELAR</button>
            <button onClick={handleApply} disabled={busy || (tab === "upload" ? !file : !url.trim())} className="px-5 py-2 bg-primary text-primary-foreground text-xs font-heading font-bold rounded disabled:opacity-40 flex items-center gap-2">{busy && <Loader2 className="w-3.5 h-3.5 animate-spin" />} {busy ? "APLICANDO..." : "APLICAR ARTE"}</button>
          </div>
        </div>
      </div>
    </div>
